...
function findPath1(){
	...
	// var marker = new Tmap.Marker(new Tmap.LonLat("126.98261605850641", "37.57528380389962").transform("EPSG:4326", "EPSG:3857"), icon);//설정한 좌표를 "EPSG:3857"로 좌표변환한 좌표값으로 설정합니다.
	// markerWaypointLayer.addMarker(marker);//마커 레이어에 마커 추가

	// var size = new Tmap.Size(24, 38);//아이콘 크기 설정
	// var offset = new Tmap.Pixel(-(size.w / 2), -size.h);//아이콘 중심점 설정
	// var icon = new Tmap.IconHtml('<img src=http://tmapapis.sktelecom.com/upload/tmap/marker/pin_b_m_p.png />', size, offset);//마커 아이콘 설정
	// var marker = new Tmap.Marker(new Tmap.LonLat("126.98773907773705", "37.56611469775449").transform("EPSG:4326", "EPSG:3857"), icon);//설정한 좌표를 "EPSG:3857"로 좌표변환한 좌표값으로 설정합니다.
	// markerWaypointLayer.addMarker(marker);//마커 레이어에 마커 추가
								
	var headers = {}; 
	headers["appKey"]=authKey;//실행을 위한 키 입니다. 발급받으신 AppKey를 입력하세요.
	headers["Content-Type"]="application/json";
	var urlStr = "https://api2.sktelecom.com/tmap/routes/prediction?version=1&reqCoordType=WGS84GEO&resCoordType=EPSG3857&format=xml";//타임머신 자동차 길 안내 api 요청 url입니다. 요청 좌표계 유형을 WGS84GEO로 지정합니다.
	var params = JSON.stringify({
	"routesInfo": {//길 안내 요청 정보 입니다.
			"departure": {
			"name": startName,
			"lon": startX,
			"lat": startY
			...
			"predictionTime": timeAfter,
			"wayPoints": {//경유지 정보 입니다.
			"wayPoint": [
				{
				"lon": wayX,
				"lat": wayY
	...
	if (passList){
		let parsed = JSON.parse(params).routesInfo.wayPoints.wayPoint[0]
		parsed.lon = wayX
		parsed.lat = wayY
	...
	success : function(response){
		...
		document.querySelector("#main__header__result").appendChild(document.createTextNode(tDistance+tTime))
